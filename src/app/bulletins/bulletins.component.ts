...
/*
  generateBulletinPremier(eleve: any, notes: any[]) {
    ...
    if (!eleve) {
      console.error("Erreur : aucune donnée d'élève !");
      return;
    }
    ...
    doc.text("Académie de Thiès", 80, 20);
    doc.text("École: Lycée De Touba toul", 80, 25);
    // doc.setFontSize(14);
    // doc.text("BULLETIN DE NOTES", 80, 35);
    
    doc.setFontSize(14);
    doc.text("BULLETIN DE NOTES", 80, 20);

    // Informations de l'élève
    doc.setFontSize(12);
    doc.text(`Prénom(s) : ${eleve.prenom || 'N/A'}`, 10, 30);
    doc.text(`Nom : ${eleve.nom || 'N/A'}`, 100, 30);
    doc.text(`Né(e) le : ${eleve.date_naissance || 'N/A'}`, 10, 40);
    doc.text(`Classe : ${eleve.nom_classe  || 'N/A'}`, 100, 40);
    // doc.text(`Matricule : ${eleve.matricule}`, 10, 50);
    ...
    autoTable(doc, {
      ...
      body: notes.map(note => [
        ...
        note.composition,
        note.coefficient,
        ((parseFloat(note.devoir1) + parseFloat(note.devoir2) + parseFloat(note.composition)) / 3).toFixed(2)
      ]),
    });
    ...
    let totalCoef = 0;
    notes.forEach(note => {
      const moyenneMatiere = (parseFloat(note.devoir1) + parseFloat(note.devoir2) + parseFloat(note.composition)) / 3;
      totalMoyenne += moyenneMatiere * parseFloat(note.coefficient);
      totalCoef += parseFloat(note.coefficient);
    });
    ...
    // doc.text(`Moyenne Générale : ${moyenneGenerale}/20`, 10, doc.lastAutoTable.finalY + 10);
    doc.text(`Moyenne Générale : ${moyenneGenerale}/20`, 10, (doc as any).lastAutoTable.finalY + 10);

    // Sauvegarde ou affichage du PDF
    doc.save(`Bulletin_${eleve.prenom}_${eleve.nom}_${this.selectedEleveId}.pdf`);
  }
*/
  ...
  generateBulletinPremier(eleve: any, notes: any[]) {
    ...
    autoTable(doc, {
      ...
      body: notes.map((note: any) => [
        ...
        ((parseFloat(note.devoir1) + parseFloat(note.devoir2) + parseFloat(note.composition)) / 3).toFixed(2),
        note.coefficient,
        (( (parseFloat(note.devoir1) + parseFloat(note.devoir2) + parseFloat(note.composition)) / 3) * parseFloat(note.coefficient) ).toFixed(2),
        "T.H",
        note.rang,
        note.appreciation
      ]),
    ...
    let totalCoef = 0;
    notes.forEach(note => {
      const moyenneMatiere = (parseFloat(note.devoir) + parseFloat(note.composition)) / 2;
      totalMoyenne += moyenneMatiere * parseFloat(note.coefficient);
      totalCoef += parseFloat(note.coefficient);
    });
    let moyenneGenerale = totalCoef ? (totalMoyenne / totalCoef).toFixed(2) : "N/A";
